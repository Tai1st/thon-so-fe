...
function validateMemberForm(form: {
  ...
  if (!CCCD_RE.test(form.cccd.trim())) return 'Số Căn Cước phải gồm đúng 12 chữ số.';
  if (form.phone.trim() && !PHONE_RE.test(form.phone.trim())) return 'Số điện thoại phải gồm đúng 10 chữ số (hoặc để trống).';
  return null;
}
...
export function FamilyTab({
  ...
  async function saveHouseNumber() {
    try {
      const res = await clientApi<{ houseNumber: string }>('households/me/house-number', {
        method: 'PATCH',
      ...
      showNotice('error', err instanceof ClientApiError ? err.message : 'Không thể lưu số nhà.');
    }
  }
  ...
  function simulateGPSUpdate() {
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        try {
          const res = await clientApi<{ gpsCoord: { lat: number; lng: number } }>('households/me/gps', {
            method: 'PATCH',
            body: { lat: position.coords.latitude, lng: position.coords.longitude },
          });
          onHouseholdChange({ ...household, gpsCoord: res.gpsCoord });
          showNotice('success', 'Đã lấy vị trí GPS hiện tại của thiết bị và lưu vào hồ sơ hộ gia đình.');
        } catch (err) {
          showNotice('error', err instanceof ClientApiError ? err.message : 'Không thể lưu vị trí GPS.');
        } finally {
          setGpsBusy(false);
        }
      },
  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-4 text-left sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h4 className="font-serif text-lg font-bold text-stone-900">Quản lý Thành Viên &amp; Tọa độ Gia đình</h4>
          <p className="text-xs text-stone-500">Cơ chế quản lý khép kín, đảm bảo tính bảo mật và toàn vẹn dữ liệu cá nhân.</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 items-center gap-6 border-t border-stone-200 pt-6 text-left lg:grid-cols-12">
        <div className="space-y-4 lg:col-span-5">
          ...
          <div className="space-y-1.5 rounded-xl border border-stone-200 bg-stone-50 p-3.5 font-mono text-xs text-stone-600">
            <div className="flex justify-between">
              ...
              <span className="font-bold text-stone-900">
                {household.gpsCoord ? `${household.gpsCoord.lat.toFixed(6)}° N` : 'Chưa định vị'}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-stone-400">Kinh độ (Longitude):</span>
              <span className="font-bold text-stone-900">
                {household.gpsCoord ? `${household.gpsCoord.lng.toFixed(6)}° E` : 'Chưa định vị'}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-stone-400">Trạng thái định vị:</span>
              <span className={`font-bold ${household.gpsCoord ? 'text-emerald-600' : 'text-amber-600'}`}>
                {household.gpsCoord ? 'Đã định vị bằng GPS thiết bị' : 'Chưa có dữ liệu'}
              </span>
            </div>
          </div>
          ...
          >
            <i className="fa-solid fa-location-crosshairs animate-pulse text-primary-400" /> Định vị GPS hiện tại
          </button>

          <a
            href={household.gpsCoord ? `https://www.google.com/maps?q=${household.gpsCoord.lat},${household.gpsCoord.lng}` : undefined}
            ...
            className={`flex w-full items-center justify-center gap-2 rounded-xl bg-emerald-50 py-3 text-xs font-bold uppercase tracking-wider text-emerald-600 transition-all ${
              household.gpsCoord ? 'hover:bg-emerald-100' : 'cursor-not-allowed opacity-40'
            }`}
          >
          ...
          <div className="space-y-1.5 pt-2">
            <label className="block text-[10px] font-bold uppercase tracking-wider text-stone-500">Số nhà / Địa chỉ cụ thể</label>
      ...
      {editingMember && (
        <EditMemberModal
          ...
          onSuccess={async () => {
            ...
            showNotice('info', 'Yêu cầu điều chỉnh thông tin đã được gửi và đang chờ Admin phê duyệt.');
          }}
        />
      )}
      ...
      {addingMember && (
        <AddMemberModal
          ...
          onSuccess={async () => {
            ...
            await refreshRequests();
            showNotice('info', 'Yêu cầu thêm thành viên mới đã được gửi và đang chờ Admin phê duyệt.');
          }}
        />
      )}
    </>
...
function EditMemberModal({
  ...
  async function submit(e: React.FormEvent) {
    ...
    const changed = (Object.keys(initial) as (keyof typeof initial)[]).some((k) => form[k] !== initial[k]);
    if (!changed) {
    ...
    try {
      ...
      setError(err instanceof ClientApiError ? err.message : 'Không thể gửi yêu cầu.');
    } finally {
      setSubmitting(false);
    }
  }
    ...
    <div className="fixed inset-0 z-[1100] flex items-center justify-center bg-stone-950/80 p-4">
      <div className="flex max-h-[90vh] w-full max-w-md flex-col overflow-hidden rounded-3xl border border-stone-200 bg-white shadow-2xl">
        ...
        <form onSubmit={submit} className="flex-1 space-y-3 overflow-y-auto p-6">
          <FormField label="Họ và tên *" value={form.name} onChange={(v) => setForm({ ...form, name: v })} />
          <FormField label="Quan hệ với chủ hộ *" value={form.relation} onChange={(v) => setForm({ ...form, relation: v })} />
          <div className="grid grid-cols-2 gap-3">
            <FormDate label="Ngày sinh *" value={form.dob} onChange={(v) => setForm({ ...form, dob: v })} />
            <FormGender value={form.gender} onChange={(v) => setForm({ ...form, gender: v })} />
          </div>
          <FormField
            label="Số Căn Cước (12 số) *"
            value={form.cccd}
            onChange={(v) => setForm({ ...form, cccd: v.replace(/\D/g, '').slice(0, 12) })}
          />
          <FormField
          ...
          <p className="text-[11px] text-stone-400">
            (*) Bắt buộc nhập. Yêu cầu sẽ ở trạng thái &quot;Chờ duyệt&quot; cho đến khi Admin phê duyệt.
          </p>
  ...
  return (
    <div className="fixed inset-0 z-[1100] flex items-center justify-center bg-stone-950/80 p-4">
      <div className="flex max-h-[90vh] w-full max-w-md flex-col overflow-hidden rounded-3xl border border-stone-200 bg-white shadow-2xl">
        ...
        <form onSubmit={submit} className="flex-1 space-y-3 overflow-y-auto p-6">
          <FormField label="Họ và tên *" value={form.name} onChange={(v) => setForm({ ...form, name: v })} />
          <FormField label="Quan hệ với chủ hộ *" placeholder="VD: Con, Vợ/Chồng..." value={form.relation} onChange={(v) => setForm({ ...form, relation: v })} />
          <div className="grid grid-cols-2 gap-3">
            <FormDate label="Ngày sinh *" value={form.dob} onChange={(v) => setForm({ ...form, dob: v })} />
            <FormGender value={form.gender} onChange={(v) => setForm({ ...form, gender: v })} />
          </div>
  ...
  return (
    <div className="space-y-1.5">
      ...
      <input
        ...
        onChange={(e) => onChange(e.target.value ? isoToDmy(e.target.value) : '')}
        className="w-full rounded-xl border border-stone-200 bg-stone-50 px-3 py-2 text-xs text-stone-800 outline-none transition-colors focus:border-primary-500"
      />
    </div>
  );
...
function FormGender({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  return (
    <div className="space-y-1.5">
      ...
      <div className="flex h-[34px] items-center gap-4 rounded-xl border border-stone-200 bg-stone-50 px-3 text-xs text-stone-800">
        <label className="flex items-center gap-1.5">
          <input type="radio" name="gender" checked={value === 'male'} onChange={() => onChange('male')} /> Nam
        </label>
        <label className="flex items-center gap-1.5">
          <input type="radio" name="gender" checked={value === 'female'} onChange={() => onChange('female')} /> Nữ
        </label>
      </div>
    </div>
  );
